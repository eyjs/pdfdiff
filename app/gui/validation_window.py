...
class ValidationWindow:
    ...
    def _setup_ui(self):
        ...
        # OCR 엔진 선택 드롭다운 추가
        ttk.Label(control_frame, text="OCR 엔진:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        self.ocr_combobox.grid(row=2, column=1, sticky=tk.EW, padx=5, pady=5)

        self.target_label = ttk.Label(control_frame, text="검사 대상 파일:")
        self.target_label.grid(row=3, column=0, sticky=tk.W, pady=5)
        self.path_var = tk.StringVar()
        ttk.Entry(control_frame, textvariable=self.path_var, state="readonly").grid(row=3, column=1, sticky=tk.EW, padx=5, pady=5)
    ...
    def _create_scrolled_canvas(self, parent):
        ...
        canvas.bind("<ButtonPress-2>", self.controller.start_pan)
        canvas.bind("<B2-Motion>", self.controller.do_pan)
        canvas.bind("<ButtonRelease-2>", self.controller.end_pan)
