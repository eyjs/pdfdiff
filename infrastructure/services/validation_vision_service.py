...
class ValidationVisionService:
    ...
    def validate_roi(self, original_doc, filled_doc, field_name, roi_info):
        ...
        try:
            ...
            # --- 4. 검증 로직 분기 ---
            if method == "ocr":
                # OCR은 채워진 ROI만 사용하므로 리사이즈된 원본이 필요 없음
                self._validate_with_ocr(result, filled_roi_color, threshold)
            elif method == "contour":
    ...
    # --- 검증 헬퍼 메서드들 (구체화된 로직) ---
    def _validate_with_ocr(self, result, roi_img, threshold):
        gray_roi = cv2.cvtColor(roi_img, cv2.COLOR_BGR2GRAY)
        binary_roi = cv2.adaptiveThreshold(gray_roi, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
        raw_text = pytesseract.image_to_string(binary_roi, lang='kor+eng')
        ...
        if len(clean_text) < threshold:
            ...
            result["message"] = f"OCR OK: '{clean_text[:30]}...'"
